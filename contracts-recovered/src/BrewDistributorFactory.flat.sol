...
/// @title BrewHolderDistributor
/// @notice Routes a launch's creator fee share to ALL holders, trustlessly.
///
/// Brew tokens are deliberately hook-free — that is what makes them tradeable
...
contract BrewHolderDistributor is IPancakeV3SwapCallback, ReentrancyGuard {
    using SafeERC20 for IERC20;

    /// @notice launch on brew.family
    string public constant BREW = "launch on brew.family";
    ...
    constructor(BrewFactory brewFactory_, address token_) {
...
contract BrewDistributorFactory {
    ...
    BrewFactory public immutable brewFactory;
    ...
    constructor(BrewFactory brewFactory_) {
    ...
    function create(address token) public returns (address distributor) {
        ...
        distributor = address(
            new BrewHolderDistributor{salt: bytes32(uint256(uint160(token)))}(brewFactory, token)
        );
    ...
    function distribute(address token, uint256 minTokensOut)
        ...
        returns (uint256 quoteSpent, uint256 tokensBurned)
    {
        return BrewHolderDistributor(create(token)).distribute(minTokensOut);
    }
    ...
    function predict(address token) external view returns (address) {
        bytes32 initCodeHash = keccak256(
            abi.encodePacked(type(BrewHolderDistributor).creationCode, abi.encode(brewFactory, token))
        );
        return address(
            uint160(
                uint256(
