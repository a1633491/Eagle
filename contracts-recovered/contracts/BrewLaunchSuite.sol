...
///   1. deploys a fixed-supply, hook-free EagleToken via CREATE2;
...
///   4. locks every LP NFT forever in the EagleLiquidityLocker, which streams
...
/// @dev Minimal PancakeSwap V3 pool surface used by Eagle.
...
/// the first block, which is what makes Eagle launches universally tradeable.
...
contract EagleLiquidityLocker is IERC721Receiver, ReentrancyGuard {
    ...
    struct LockedPosition {
        /// @dev EagleToken this position belongs to (zero until assigned).
        address token;
    ...
    /// @notice The Eagle factory allowed to assign fee routing for new positions.
    ...
    /// @notice Pull-based fee balances for manual recipients such as creators.
...
/// @dev Minimal PancakeSwap V3 factory surface used by Eagle.

contract EagleToken is ERC20 {
    /// @notice launch on eagle.family
    string public constant EAGLE = "launch on eagle.family";

    /// @notice The wallet that launched this token through the Eagle factory.
    address public immutable creator;

    /// @notice The Eagle factory that deployed this token.
    address public immutable brewFactory;
...
/// @title EagleHolderDistributor
/// @notice Routes a launch's creator fee share to ALL holders, trustlessly.
///
/// Eagle tokens are deliberately hook-free — that is what makes them tradeable
...
contract EagleHolderDistributor is IPancakeV3SwapCallback, ReentrancyGuard {
    ...
    /// @param minTokensOut Slippage floor for the burned amount; pass a value
    /// quoted off-chain to keep sandwich damage bounded.
